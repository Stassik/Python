...
``` python
# Пример 1
list_1 = [x for z in range(1, 20)]
list_1 = list(map(lambda x: x + 10, list_1 ))

# Пример 2
data = "12 34 23 4 565 3"
data = list(map(int, data.split())) #Вывод: [12, 34, 23, 4, 565, 3]
...
``` python
# Пример 1
data = "12 35 23 4 565 3"
data = list(filter(lambda x: x % 10 == 5, data)) #Вывод: [35, 565]
...
1. `а` - открытие для добавления данных
    - позволяет дописывать что-то в имеющиеся файл;
    - если вы попробуете дописать что-то в несуществующий файл, то файл будет создан т в него нанется запись
2. `r` - открытие для чтения данных:
    - Позволяет читать данные с файла;
    - Если вы попробуете считать данные с несуществующего файла, программа выдаст ошибку
...
5. `r+`:
    - Позволяет открыть файл для чтения и дописывать в него;
    - Если файла не существует, программа выдаст ошибку

```python
colors = ['red', 'green', 'blue']
data = open('file.txt', 'a') #указываем режи в котором будем работать
